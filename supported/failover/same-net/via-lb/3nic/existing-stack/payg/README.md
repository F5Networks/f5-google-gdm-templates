# Deploying the BIG-IP VE in Google Cloud - (frontend via GLB)- HA Cluster (Active/Standby): Existing Stack with PAYG Licensing
...
This solution uses a Google Deployment Manager (GDM) Template to launch two BIG-IP VEs in an Active/Standby configuration with network failover enabled in a Google Virtual Private Cloud, using PAYG (pay as you go) hourly (utility) licensing.
...
The following are prerequisites for the F5 failover-lb 3 NIC GDM template:
...
- PAYG: [**f5-existing-stack-same-net-cluster-payg-3nic-bigip.yaml**](f5-existing-stack-same-net-cluster-payg-3nic-bigip.yaml)
