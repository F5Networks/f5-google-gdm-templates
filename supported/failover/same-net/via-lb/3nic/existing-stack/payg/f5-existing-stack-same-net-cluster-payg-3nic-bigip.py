...
def Metadata(context,group, storageName, licenseType):
  ...
  if ALLOWUSAGEANALYTICS in ['yes', 'true']:
      ...
      SENDANALYTICS = ' --metrics "cloudName:google,region:' + context.properties['region'] + ',bigipVersion:' + context.properties['imageName'] + ',customerId:${CUSTOMERID},deploymentId:${DEPLOYMENTID},templateName:f5-existing-stack-same-net-cluster-payg-3nic-bigip.py,templateVersion:3.18.0,licenseType:payg"'
  else:
