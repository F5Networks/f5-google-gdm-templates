# Deploying the BIG-IP VE in Google Cloud - (frontend via GLB)- HA Cluster (Active/Standby): Existing Stack with BYOL Licensing
...
This solution uses a Google Deployment Manager (GDM) Template to launch two BIG-IP VEs in an Active/Standby configuration with network failover enabled in a Google Virtual Private Cloud, using BYOL (bring your own license) (utility) licensing.
