...
def Metadata(context, group, storageName, licenseType):
  ...
  if ALLOWUSAGEANALYTICS == "yes":
    ...
    SENDANALYTICS = ' --metrics "cloudName:google,region:' + context.properties['region'] + ',bigipVersion:' + context.properties['imageName'] + ',customerId:${CUSTOMERID},deploymentId:${DEPLOYMENTID},templateName:f5-existing-stack-same-net-cluster-payg-3nic-bigip.py,templateVersion:v2.1.0,licenseType:payg"'
  else:
  ...
  metadata = {
            'items': [{
                ...
                'value': ('\n'.join(['#!/bin/bash',
                                    ...
                                    CUSTHASH,
                                    'nohup /config/waitThenRun.sh f5-rest-node /config/cloud/gce/node_modules/@f5devcentral/f5-cloud-libs/scripts/onboard.js --signal ONBOARD_DONE --wait-for NIC_SWAP_DONE --output /var/log/cloud/google/onboard.log --log-level debug --install-ilx-package file:///config/cloud/f5-appsvcs-3.5.1-5.noarch.rpm --host localhost --user admin --password-url file:///config/cloud/gce/.adminPassword --password-encrypted --port 443 --ssl-port ' + context.properties['mgmtGuiPort'] + ' --hostname $(curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/hostname\" -H \"Metadata-Flavor: Google\") --ntp 0.us.pool.ntp.org --ntp 1.us.pool.ntp.org --tz UTC --module ltm:nominal ' + LICENSE + SENDANALYTICS + ' &>> /var/log/cloud/google/install.log < /dev/null &',
...
def Instance(context, group, storageName, licenseType):
  ...
  instance = {
        ...
        'canIpForward': True,
        'tags': {
          'items': ['intfw-' + context.env['deployment']]
        },
        'labels': {
