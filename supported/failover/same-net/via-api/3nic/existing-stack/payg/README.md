# Deploying the BIG-IP in Google Cloud - HA Cluster (Active/Standby): Existing Stack with PAYG Licensing
...
This solution uses a Google Deployment Manager (GDM) Template to launch two BIG-IP VEs in an Active/Standby configuration with network failover enabled (the template also supports [multiple traffic groups](#traffic-group-configuration) which enables Active/Active deployments) in a Google Virtual Private Cloud, using PAYG (pay as you go) hourly licensing.
...
- This template creates target instances for each BIG-IP VE as well as a user specified number of forwarding rules.  For more information on creating or modifying these rules see the google documentation [here](https://cloud.google.com/load-balancing/docs/forwarding-rules).
...
- [**f5-deployment-template.yaml**](f5-existing-stack-same-net-cluster-payg-3nic-bigip.yaml)
...
| aliasIp | Yes | Alias IP address(es) to be used for application traffic, including CIDR suffix. The address(es) must belong to the subnet noted above in key 'subnet1'. A list of alias IPs can be provided, separated by a semi-colonm for example **10.x.x.16/28;10.x.x.32/28** |
| numberOfForwardingRules | Yes | Number of forwarding rules to create, for example '1'.  All integers from 0 to the max quota for the forwarding rules resource type are allowed. |
...
After you have edited the YAML file with the appropriate values, save the YAML file in a location accessible from the gcloud command line.  Save the [python file](f5-existing-stack-same-net-cluster-payg-3nic-bigip.py) in the same location.
