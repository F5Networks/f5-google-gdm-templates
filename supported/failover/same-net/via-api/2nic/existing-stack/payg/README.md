# Deploying the BIG-IP VE in Google Cloud - HA Cluster (Active/Standby): Existing Stack with PAYG Licensing
...
The following are prerequisites for the F5 failover-api 2 NIC GDM template:
...
- Two Virtual Private Cloud (VPC) networks with at least one subnet in each. The subnet for the management network requires a route and access to the Internet for the initial configuration to download the BIG-IP cloud library.
...
- PAYG: [**f5-existing-stack-same-net-cluster-payg-2nic-bigip.yaml**](f5-existing-stack-same-net-cluster-payg-2nic-bigip.yaml)
...
After you have edited the YAML file with the appropriate values, save the YAML file in a location accessible from the gcloud command line.  Save the [python file](f5-existing-stack-same-net-cluster-payg-2nic-bigip.py) in the same location.
