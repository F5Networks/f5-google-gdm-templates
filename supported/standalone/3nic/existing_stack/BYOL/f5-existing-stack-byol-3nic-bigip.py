          ...
          'metadata': {
              'items': [{
                  ...
                  'value': (''.join(['#!/bin/bash\n',
                                    ...
                                    '# Grab ip subnetmask assined to 1.1\n',
                                    'INT1SUBNETMASK=`curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/1/subnetmask\" -H \"Metadata-Flavor: Google\"`\n',
                                    ...
                                    '# Grab ip subnetmask assined to 1.2\n',
                                    'INT2SUBNETMASK=`curl -s -f --retry 20 \"http://metadata.google.internal/computeMetadata/v1/instance/network-interfaces/2/subnetmask\" -H \"Metadata-Flavor: Google\"`\n',
                                    ...
                                    '   echo $(( $2 + (${#x}/4) ))\n',
                                    '}\n',
                                    'INT1MASK=`mask2cdr ${INT1SUBNETMASK}`\n',
                                    'INT2MASK=`mask2cdr ${INT2SUBNETMASK}`\n',
                                    'IFS=. read -r i1 i2 i3 i4 <<< ${INT1ADDRESS}\n',
                                    'IFS=. read -r m1 m2 m3 m4 <<< ${INT1SUBNETMASK}\n',
                                    'network=`printf "%d.%d.%d.%d\n" "$((i1 & m1))" "$((i2 & m2))" "$((i3 & m3))" "$((i4 & m4))"`\n',
                                    'GATEWAY=$(echo "`echo $network |cut -d"." -f1-3`.$((`echo $network |cut -d"." -f4` + 1))")\n',
                                    'IFS=. read -r i1 i2 i3 i4 <<< ${INT2ADDRESS}\n',
                                    'IFS=. read -r m1 m2 m3 m4 <<< ${INT2SUBNETMASK}\n',
                                    ...
                                    '\"tmsh create net route ext_rt network ${network}/${INT1MASK} gw ${GATEWAY}\"\n',
                                    ...
                                    '\"tmsh create net route int_rt network ${network2}/${INT2MASK} gw ${GATEWAY2}\"\n',
