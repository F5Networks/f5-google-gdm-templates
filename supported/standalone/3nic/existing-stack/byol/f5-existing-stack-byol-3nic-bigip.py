...
def Metadata(context):
    ...
    if ALLOWUSAGEANALYTICS == 'yes':
        ...
        SENDANALYTICS = ' --metrics "cloudName:google,region:' + context.properties['region'] + ',bigipVersion:' + context.properties['imageName'] + ',customerId:${CUSTOMERID},deploymentId:${DEPLOYMENTID},templateName:f5-existing-stack-byol-3nic-bigip.py,templateVersion:3.2.0,licenseType:byol"'
    else:
    ...
    metadata = {
                'items': [{
                    ...
                    'value': ('\n'.join(['#!/bin/bash',
                                    ...
                                    'nohup /config/waitThenRun.sh f5-rest-node /config/cloud/gce/node_modules/@f5devcentral/f5-cloud-libs/scripts/onboard.js --host localhost --port ' + str(context.properties['mgmtGuiPort']) + ' -o /var/log/cloud/google/onboard.log --wait-for INT_COLLECTION_DONE --signal LICENSING_DONE --log-level ' + str(context.properties['logLevel']) + ' --license ' + str(context.properties['licenseKey1']) + ' >> /var/log/cloud/google/install.log < /dev/null ',
                                    'nohup /config/waitThenRun.sh f5-rest-node /config/cloud/gce/node_modules/@f5devcentral/f5-cloud-libs/scripts/onboard.js --db provision.managementeth:eth1 --host localhost --wait-for LICENSING_DONE --signal FIRST_BOOT_COMPLETE --force-reboot >> /var/log/cloud/google/install.log < /dev/null',
                                    'else',
