...
def Metadata(context):
    ...
    if ALLOWUSAGEANALYTICS:
        ...
        SENDANALYTICS = ' --metrics "cloudName:google,region:' + context.properties['region'] + ',bigipVersion:' + context.properties['imageName'] + ',customerId:${CUSTOMERID},deploymentId:${DEPLOYMENTID},templateName:f5-existing-stack-payg-1nic-bigip.py,templateVersion:v2.1.1,licenseType:payg"'
    else:
    ...
    metadata = {
                'items': [{
                    ...
                    'value': ('\n'.join(['#!/bin/bash',
                                    ...
                                    '       if [[ $? == 0 ]]; then',
                                    '           wait_for_ready',
                                    '           response_code=$(/usr/bin/curl -sku admin:$passwd -w "%{http_code}" -X POST https://localhost:${mgmtGuiPort}/mgmt/shared/appsvcs/declare -d @$file_loc -o /dev/null)',
