...
- Note: If you are unable to delete the GDM deployment with an error similar to 'The target_instance resource <resource> is already being used by forwarding rule <resource>', try again to delete the deployment.  If it continues to fail delete any forwarding rules created by the deployment and try again.
