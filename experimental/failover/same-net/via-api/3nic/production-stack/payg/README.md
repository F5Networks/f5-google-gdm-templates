# Deploying the BIG-IP in Google Cloud - HA Cluster (Active/Standby): Production Stack with PAYG Licensing
...
- PAYG: [**f5-deployment-template.yaml**](payg/f5-deployment-template.yaml)
...
| aliasIp | Yes | Alias IP address(es) to be used for application traffic, including CIDR suffix. The address(es) must belong to the subnet noted above in key 'subnet1'. A list of alias IPs can be provided, seperated by a semi-colonm for example **10.x.x.16/28;10.x.x.32/28** |
