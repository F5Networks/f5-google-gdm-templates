# Deploying the BIG-IP in Google Cloud (frontend via GLB)- HA Cluster (Active/Standby): Existing Stack with BYOL Licensing
...
This solution uses a Google Deployment Manager (GDM) Template to launch two BIG-IP VEs in an Active/Standby configuration with network failover enabled in a Google Virtual Private Cloud, using BYOL (bring your own license) licensing.

Google load balancers monitor the BIG-IP VEs to determine which unit is active. Traffic is sent to active BIG-IP which flows from the BIG-IP VE(s) to the application servers. Each BIG-IP VE has 3 network interfaces (NICs), one for management, one for external traffic, and one for internal traffic.
...
- [**f5-deployment-template.yaml**](https://github.com/F5Networks/f5-google-gdm-templates/blob/master/experimental/failover/same-net/via-api/3nic/existing-stack/byol/f5-deployment-template.yaml)
